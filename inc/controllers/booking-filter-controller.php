...
class BookingFilterController
{
    ...
    public static function getMasters()
    {
        ...
        if ($masters_query->have_posts()) {
            while ($masters_query->have_posts()) {
                ...
                $specialization = get_post_meta($post_id, 'specialization', true) ?: (isset(self::MASTER_LEVELS[$level]) ? self::MASTER_LEVELS[$level] : '');
    ...
    public static function getMasterDetails()
    {
        ...
        } else if (class_exists('AltegioClient')) {
     ...
     */
    private static function checkMasterAvailability($master_altegio_id, $service_altegio_ids, $check_date)
    {
        if (!class_exists('AltegioClient')) {
        ...
        try {
            ...
            error_log("Altegio availability check error for master $master_altegio_id: " . $e->getMessage());
        }
    ...
    private static function generateFallbackTimeSlots($date)
    {
        ...
        for ($hour = $start_hour; $hour < $end_hour; $hour++) {
            for ($min = 0; $min < 60; $min += $interval) {
                ...
                if (mt_rand(0, 10) > 2) { // 80% chance of being available
