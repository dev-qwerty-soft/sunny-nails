...
function altegio_submit_booking()
{
    ...
    $required_fields = array('service_id', 'staff_id', 'date', 'time', 'client_name', 'client_phone');
    ...
    // Try to submit booking to Altegio API
    if (class_exists('AltegioClient')) {
        try {
    ...
    // If we reach here, use fallback response
...
// Register AJAX handlers for submitting bookings
