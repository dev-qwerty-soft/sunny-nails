...
function altegio_submit_booking()
{
  ...
  $adjustment_percent = isset($_POST['adjustment_percent'])
    ? (float) $_POST['adjustment_percent']
    : 0;
  ...
  $client_comment = isset($_POST['client_comment'])
  ...
  // Log the API data
  error_log(
    'Submitting booking to Altegio with price data: ' .
      json_encode([
    ...
    try {
      ...
      if (isset($result['success']) && $result['success']) {
        ...
        $booking_id = save_booking_record_with_price([
          ...
          'adjusted_price' => $total_price,
          'price_adjustment' => $price_adjustment,
          'altegio_reference' => isset($result['data'][0]['record_id'])
            ? $result['data'][0]['record_id']
            : '',
        ]);
        ...
        wp_send_json_success([
          ...
          'booking' => [
            'reference' => isset($result['data'][0]['record_id'])
              ? 'BK' . $result['data'][0]['record_id']
              : 'BK' . mt_rand(10000, 99999),
            'datetime' => $datetime,
            'adjusted_price' => $total_price,
        ...
        $error_message = isset($result['error'])
          ? $result['error']
          : 'Error connecting to booking service';
...
function save_booking_record_with_price($booking_data)
{
  ...
  if (!$post_id || is_wp_error($post_id)) {
    error_log(
      'Failed to create booking record: ' . ($post_id->get_error_message() ?? 'Unknown error'),
    );
    return false;
  }
...
function altegio_get_filtered_staff()
{
  ...
  if ($query->have_posts()) {
    while ($query->have_posts()) {
      ...
      if (!array_intersect($service_ids, $related_services)) {
        continue;
      }
          ...
          <div class="service-info">
            <div class="service-title">
              ...
              <div class="service-checkbox-wrapper">
                <div class="service-price"><?php echo esc_html($price); ?> <?php echo esc_html(
                                                                              $currency,
                                                                            ); ?></div>
                <input type="checkbox"
...
function ajax_get_time_slots()
{
  ...
  error_log(
    'Sending to API getBookTimes: staff_id=' .
      $staff_id .
      ', date=' .
      $date .
      ', service_ids=' .
      json_encode($altegio_service_ids),
  );
...
function get_services_for_master()
{
  ...
  if (!isset($_POST['staff_id']) || empty($_POST['staff_id'])) {
    // Staff ID is missing
    wp_send_json_error(['message' => 'Staff ID is missing.']);
    return;
  }
  ...
  if ($services) {
    wp_send_json_success(['data' => $services]);
  } else {
    wp_send_json_error(['message' => 'No services found for this staff.']);
  }
}
...
function handle_get_month_availability()
{
  ...
  if (empty($staff_id) || empty($start_date_str)) {
    error_log(
      "Month availability: Missing parameters - staff_id={$staff_id}, start_date={$start_date_str}",
    );
  ...
  error_log(
    "Month availability: Calling Altegio API for staff {$staff_id} from {$start_date->format(
      'Y-m-d',
  ...
  error_log(
    "Month availability: Successfully processed {$days_in_month} days, {$result_data['working_days_count']} working days",
  );
...
function handle_clear_month_availability_cache()
{
  ...
  if ($staff_id && $month) {
    ...
    global $wpdb;
    $wpdb->query(
      "DELETE FROM {$wpdb->options} WHERE option_name LIKE '%altegio_month_availability_%'",
    );
...
add_action(
  'wp_ajax_nopriv_clear_month_availability_cache',
  'handle_clear_month_availability_cache',
);
...
function sunny_get_month_availability()
{
  ...
  foreach ($service_ids as $sid) {
    if (intval($sid) > 1000000) {
      ...
      if ($aid) {
        $altegio_service_ids[] = intval($aid);
      }
    }
  }
...
function ajax_get_time_slots_for_all_masters()
{
  ...
  $service_ids = isset($_POST['service_ids'])
    ? array_map('intval', (array) $_POST['service_ids'])
    : [];
...
add_action('wp_ajax_get_next_seances', 'ajax_get_next_seances');
add_action('wp_ajax_nopriv_get_next_seances', 'ajax_get_next_seances');

function ajax_get_next_seances()
{
  $staff_id = isset($_POST['staff_id']) ? sanitize_text_field($_POST['staff_id']) : null;
  if (!$staff_id) {
    wp_send_json_error(['message' => 'Missing staff_id']);
  }
  $company_id = '1275515';
  $url = "https://api.alteg.io/api/v1/book_staff_seances/{$company_id}/{$staff_id}/";
  $response = wp_remote_get($url, [
    'headers' => [
      'Authorization' => 'Bearer 89ubt5s6rw7tcamfuuwa, User 194f0b9cd822b74b55ea0f2171357f8b',
  ...
  if (!isset($body['data']['seances'])) {
    wp_send_json_error(['message' => 'Invalid response']);
  }
  wp_send_json_success([
    'seance_date' => $body['data']['seance_date'],
    'seances'     => $body['data']['seances']
  ]);
}
