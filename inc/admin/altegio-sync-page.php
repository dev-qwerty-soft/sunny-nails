...
function enhanced_force_sync_altegio_services() {
    ...
    if ($post_id) {
      ...
      if (function_exists('update_field')) {
        ...
        foreach ($acf_fields as $field_name => $field_value) {
          ...
          if ($result) {
            error_log(
              "ACF field FORCE UPDATED: {$field_name} = {$field_value} for post {$post_id}",
            );
          } else {
  ...
  foreach ($wp_services_with_altegio_id as $wp_service) {
    $wp_altegio_id =
      get_post_meta($wp_service->ID, 'altegio_id', true) ?:
      get_post_meta($wp_service->ID, '_altegio_id', true);

    if ($wp_altegio_id && !in_array($wp_altegio_id, $altegio_service_ids)) {
      wp_delete_post($wp_service->ID, true);
      error_log(
        "Service DELETED: ID {$wp_service->ID} (Altegio ID: {$wp_altegio_id}) - no longer exists in Altegio",
      );
    }
  }
...
function enhanced_force_sync_altegio_masters() {
    ...
    if ($post_id) {
      ...
      if (function_exists('update_field')) {
        ...
        foreach ($acf_fields as $field_name => $field_value) {
          ...
          if ($result) {
            error_log(
              "Master ACF field FORCE UPDATED: {$field_name} = {$field_value} for post {$post_id}",
            );
          } else {
      ...
      if (!empty($avatar_url)) {
        ...
        if (!is_wp_error($attachment_id)) {
          ...
          error_log(
            "Master avatar FORCE UPDATED: Post ID {$post_id}, Attachment ID {$attachment_id}",
          );
        } else {
  ...
  foreach ($wp_masters_with_altegio_id as $wp_master) {
    ...
    if ($wp_altegio_id && !in_array($wp_altegio_id, $altegio_master_ids)) {
      ...
      error_log(
        "Master DELETED: ID {$wp_master->ID} (Altegio ID: {$wp_altegio_id}) - no longer exists in Altegio",
      );
    }
  }
...
function enhanced_force_sync_altegio_categories() {
  ...
  foreach ($categories['data'] as $category_data) {
    ...
    if ($existing_by_name && !is_wp_error($existing_by_name)) {
      ...
      $existing_altegio_id = get_term_meta(
        $existing_by_name->term_id,
        '_altegio_category_id',
        true,
      );
      ...
      if (empty($existing_altegio_id)) {
        ...
        error_log(
          "NAME CONFLICT DETECTED: '{$title}' exists with different Altegio ID. Creating as '{$unique_title}'",
        );
  ...
  foreach ($all_synced_categories as $category) {
    ...
    if ($category_altegio_id && !in_array($category_altegio_id, $altegio_category_ids)) {
      ...
      if (!is_wp_error($result) && $result !== false) {
        $stats['deleted']++;
        error_log(
          "DELETED obsolete category: {$category->name} (Term ID: {$category->term_id}, Altegio ID: {$category_altegio_id})",
        );
      } else {
  ...
  ?>
    <div class="wrap">
        ...
        <div class="altegio-stats" style="margin-top: 30px; padding: 20px; background: #f8f8f8; border-radius: 5px;">
            ...
            <p><strong>Categories:</strong> <?php echo is_wp_error($cat_count)
              ? '0'
              : $cat_count; ?></p>
            <p><strong>Services:</strong> <?php echo isset($service_count->publish)
              ? $service_count->publish
              : '0'; ?></p>
            <p><strong>Masters:</strong> <?php echo isset($master_count->publish)
              ? $master_count->publish
              : '0'; ?></p>
        </div>
...
function force_update_specific_field($post_id, $field_name, $field_value) {
  ...
  if (function_exists('update_field')) {
    $result = update_field($field_name, $field_value, $post_id);
    error_log(
      "FORCE UPDATE field {$field_name} = {$field_value} for post {$post_id}: " .
