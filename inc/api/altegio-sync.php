  ...
  foreach ($services['data'] as $service_data) {
    ...
    if (!is_wp_error($post_id) && $post_id) {
      ...
      update_post_meta(
        $post_id,
        'description',
        sanitize_textarea_field($service_data['comment'] ?? ''),
      );
  ...
  foreach ($staff['data'] as $master_data) {
    ...
    if (!is_wp_error($post_id) && $post_id) {
      ...
      update_post_meta(
        $post_id,
        'description',
        sanitize_textarea_field($master_data['information'] ?? ''),
      );
      update_post_meta(
        $post_id,
        'master_level',
        sanitize_text_field($master_data['specialization'] ?? '1'),
      );
      ...
      update_post_meta(
        $post_id,
        'schedule_until',
        sanitize_text_field($master_data['schedule_till'] ?? ''),
      );
...
function enhanced_sync_altegio_categories() {
  ...
  foreach ($wp_categories_with_altegio_id as $wp_category) {
    ...
    if ($wp_altegio_id && !in_array($wp_altegio_id, $altegio_category_ids)) {
      ...
      error_log(
        "Category DELETED: ID {$wp_category->term_id} (Altegio ID: {$wp_altegio_id}) - no longer exists in Altegio",
      );
    }
  }
