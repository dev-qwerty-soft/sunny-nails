...
class AltegioLogger
{
    ...
    {
        if ($level === self::ERROR) {
            error_log('ALTEGIO ERROR: ' . $message . (!empty($context) ? ' | ' . wp_json_encode($context) : ''));
        } elseif ($level === self::WARNING) {
            error_log('ALTEGIO WARNING: ' . $message . (!empty($context) ? ' | ' . wp_json_encode($context) : ''));
        } elseif ($level === self::INFO) {
            error_log('ALTEGIO INFO: ' . $message . (!empty($context) ? ' | ' . wp_json_encode($context) : ''));
        } elseif ($level === self::DEBUG && WP_DEBUG) {
            error_log('ALTEGIO DEBUG: ' . $message . (!empty($context) ? ' | ' . wp_json_encode($context) : ''));
        }
    }
}
