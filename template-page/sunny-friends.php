        ...
        <div class="sunny-friends-table-section__table">
          <div class="column main">
            ...
            if ($items && count($items) > 0 && is_array($items)) {
              foreach ($items as $item) {
                $text = isset($item['sunny_friends_main_column_table_text'])
                  ? $item['sunny_friends_main_column_table_text']
                  : '';
          ...
          if ($totalColumns !== 0 && is_array($columns) && $columns) {
            ...
            foreach ($columns as $column) {
              ...
              $title = isset($column['sunny_friends_table_column_title'])
                ? $column['sunny_friends_table_column_title']
                : '';
              $fields = isset($column['sunny_friends_table_column_field'])
                ? $column['sunny_friends_table_column_field']
                : [];
              $content = '';
              if ($fields && count($fields) > 0 && is_array($fields)) {
                foreach ($fields as $field) {
                  $text = isset($field['sunny_friends_table_column_text'])
                    ? $field['sunny_friends_table_column_text']
                    : '';
