(function ($) {
  ...
  $(document).on(
    'click',
    ".service-card .book-btn, button.book-btn[data-popup-open='true']",
    function (e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      const serviceDuration = $serviceCard
        .find('.service-duration')
        .text()
        .replace('<strong>Duration:</strong>', '')
        .trim();
      const serviceWearTime = $serviceCard
        .find('.service-wear-time')
        .text()
        .replace('<strong>Wear time:</strong>', '')
        .trim();
      ...
      setTimeout(function () {
        ...
        if ($checkbox.length) {
          ...
          $('.category-tab').each(function () {
            ...
            $checkbox = $(
              `.category-services[data-category-id="${tabId}"] .service-checkbox[data-service-id="${serviceId}"]`,
            );
        ...
        $(".booking-step[data-step='services'] .next-btn, .choose-a-master-btn").prop(
          'disabled',
          false,
        );
  ...
  $(document).on('click', "button:contains('Choose a master')", function () {
    ...
    if (
      window.bookingData &&
      (!window.bookingData.services || window.bookingData.services.length === 0)
    ) {
  ...
  $(document).on('click', '.staff-item.any-master', function () {
    debug('All masters selected');

    if (window.bookingData) {
      window.bookingData.staffId = 'any';
      window.bookingData.staffName = 'All masters';
...
(function ($) {
    ...
    function (e) {
      ...
      if (!window.bookingData.staffId && sessionStorage.getItem('selected_master_id')) {
        ...
        window.bookingData.staffName =
          sessionStorage.getItem('selected_master_name') || 'Selected Master';
      }
  ...
  $(document).on(
    'click',
    '.team-card .btn.yellow, .team-card__buttons .btn.yellow, .book-tem',
    function (e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      const masterId =
        ...
        $button.data('master-id') ||
        $masterCard.data('master-id') ||
  ...
  $(document).on('click', '.booking-step[data-step="services"] .next-btn', function (e) {
    ...
    setTimeout(() => {
      ...
      const isReady =
        ...
        services.length > 0 &&
        $(".booking-step[data-step='datetime']").hasClass('active');
